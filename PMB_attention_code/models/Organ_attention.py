...
class Organ_attention(BaseModel):
    ...
    def cal_seg_loss_attention (self,pred,gt):

        

        lmd=self.opt.SegLambda_B    
        seg_loss=lmd*self.cal_segmentation_loss(pred,gt)
    ...
    def backward_segmentation_B_stream(self):
        ...
        if self.opt.Net=='Split-Unet':
            ...
            loss_1=self.cal_segmentation_loss(pred1,gt_A)
            pred2=self.netSeg.forward_2(img_AB)
            loss_2=self.cal_segmentation_loss(pred2,gt_A)
            total_loss=loss_1+loss_2
        else:
            pred=self.netSeg(img_AB)
            total_loss=self.cal_segmentation_loss(pred,gt_A)
    ...
    def backward_G(self):
        ...
        seg_loss_B=self.cal_seg_loss(self.netSeg,fake_B,self.real_A_y) # the seg_B_loss 
